...
data class PatientDTO(
    ...
    @JsonProperty("forms") val forms: List<FormAssignmentDTO>
) {

    fun asPatient(): Patient {
        return Patient(id, name, lastName, dateOfBirth, email, joinedSince, profilePictureUrl, weeklyTurn, weeklyHour, exercises.map { it.asAssignment() }, forms.map { it.asAssignment() })
    }

    companion object {
        fun fromPatient(patient: Patient): PatientDTO {
            return PatientDTO(patient.id, patient.name, patient.lastName, patient.dateOfBirth, patient.email, patient.joinedSince, patient.avatarIndex, patient.weeklyTurn, patient.weeklyHour, listOf(), listOf())
        }
    }
}